...
class VehicleProvider extends ChangeNotifier {
  ...
  /// عدد المركبات حسب النوع (يُحسب من القائمة الكاملة)
  int getCountByType(String model) {
    return _vehicles.where((v) => v.model == model).length;
  }
    ...
    try {
      ...
      _filteredVehicles = results
          .where((v) =>
              ...
              (_typeFilter == 'all' || v.model == _typeFilter))
          .toList();
    } catch (e) {
  ...
  void _applyFilters() {
    ...
    if (_typeFilter != 'all') {
      result = result.where((v) => v.model == _typeFilter).toList();
    }
